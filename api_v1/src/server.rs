...
// The `MerkleTree` here is a trait generated from the service definition in your .proto file. It corresponds to the MerkleTree service defined in `rustle_tree.proto`.
// It is renamed as MerkleTreeTrait using as to avoid name conflicts with other items (e.g., a struct or another implementation named MerkleTree).
// MerkleTreeServer: This is the gRPC server implementation generated by tonic. It wraps an implementation of the MerkleTreeTrait and provides the necessary gRPC server logic to handle requests from clients.
use rustle_tree::{
